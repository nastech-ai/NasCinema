...
export default function MoviesPage() {
  ...
  useEffect(() => {
    apiJson<{ results: MediaItem[] }>("/api/content?category=popular-movies&type=movie&page=1")
      .then((data) => {
        const items = (data.results || []).slice(0, 10).map((item) => ({ ...item, media_type: "movie" as const }));
        setHeroItems(items);
      })
      .catch(console.error)
