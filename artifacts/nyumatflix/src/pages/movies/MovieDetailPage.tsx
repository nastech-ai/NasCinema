...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(false);
    apiJson<MediaItem>(`/api/movies/${id}`)
      .then((data) => setDetails({ ...data, media_type: "movie" }))
      .catch(() => setError(true))
  ...
  if (error) return <MediaErrorPage mediaType="movie" title="Error Loading Movie" />;
  if (!details) return <MediaNotFoundError mediaType="movie" title="Movie Not Found" />;
  ...
  const releaseDate = details.release_date
    ? new Date(details.release_date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
    : "Release Date TBA";

  return (
    <MediaDetailLayout media={[details]} mediaType="movie" isUpcoming={isUpcoming} anilistId={null} contentContainerClassName="mx-auto px-4 relative z-10 max-w-7xl !pt-4 sm:!pt-6 lg:!pt-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        <div className="lg:col-span-1">
          <div className="hidden lg:block mt-4 mb-4">
            <Poster posterPath={details.poster_path ?? undefined} title={details.title} size="large" className="rounded-lg shadow-xl" />
          </div>
          <div className="bg-black/60 backdrop-blur-md border border-white/20 rounded-lg p-6 space-y-4 shadow-xl">
            {isUpcoming && details.status && (
              <div className="flex justify-center mb-4"><StatusBadge status={details.status} /></div>
            )}
            {hasRuntime && (
            ...
            {((details.budget ?? 0) > 0 || (details.revenue ?? 0) > 0) && !isUpcoming && (
              ...
              <div className="border-t border-gray-700 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {(details.production_countries as ProductionCountry[]).map((country) => (
                    <CountryBadge key={country.iso_3166_1} country={country} variant="outline" mediaType="movie" />
                  ))}
                </div>
              </div>
            )}
            ...
            {(details.genres as Genre[] | undefined)?.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {(details.genres as Genre[]).map((genre) => (
                  <PrimaryGenreBadge key={genre.id} genreId={genre.id} genreName={genre.name} mediaType="movie" />
                ))}
              </div>
            )}
          </section>
          <MediaCarousels cast={details.credits?.cast} videos={details.videos?.results} recommendations={details.similar?.results} mediaType="movie" />
        </div>
      </div>
    </MediaDetailLayout>
  );
